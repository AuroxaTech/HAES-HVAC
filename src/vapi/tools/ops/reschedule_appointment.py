"""
HAES HVAC - Reschedule Appointment Tool

Direct Vapi tool for rescheduling existing appointments.
"""
...
async def handle_reschedule_appointment(
    ...
    command = HaelCommand(
        ...
        raw_text=conversation_context or "reschedule appointment",
    ...
    try:
        ...
        if result.status == OpsStatus.ERROR:
            return handler.format_error_response(
                ...
                "I encountered an error rescheduling your appointment. Please try again or contact us directly.",
            )
