"""
HAES HVAC - Onboarding Inquiry Tool

Direct Vapi tool for onboarding inquiries.
"""
...
async def handle_onboarding_inquiry(
    tool_call_id: str,
    ...
    """
    Handle onboarding_inquiry tool call.
    
    Parameters:
        - employee_email (optional): Employee email for specific status
        - employee_name (optional): Employee name
    """
    handler = BaseToolHandler("onboarding_inquiry")
    ...
    command = HaelCommand(
        ...
        raw_text=conversation_context or "onboarding inquiry",
    ...
    try:
        ...
        if result.status == PeopleStatus.ERROR:
            return handler.format_error_response(
                ...
                "I encountered an error retrieving onboarding information. Please try again or contact us directly.",
            )
