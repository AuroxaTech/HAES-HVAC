#!/usr/bin/env python3
"""
Create or update send_notification tool in Vapi Tools library.
"""
...
def main() -> None:
    ...
    if tool_name != "send_notification":
        print(f"Error: Expected send_notification, got: {tool_name}")
        sys.exit(1)
    ...
    payload = {
        ...
        "messages": td.get(
            "messages",
            [
                {"type": "request-start", "content": "Let me send that notification for you."},
                {"type": "request-complete", "content": ""},
                {
                    "type": "request-failed",
                    "content": "I had trouble sending that notification. I can try again.",
                },
            ],
        ),
        "function": function,
        "server": {
            "url": (td.get("server") or {}).get("url") or DEFAULT_SERVER_URL,
            "timeoutSeconds": 30,
        },
    }
    ...
    existing = next(
        (t for t in tools if (t.get("function") or {}).get("name") == "send_notification"),
        None,
    )
    ...
    if existing:
        ...
        patch_resp = httpx.patch(
            f"{VAPI_API_BASE}/tool/{tool_id}",
            headers=headers,
            json=patch_payload,
            timeout=30.0,
        )
        patch_resp.raise_for_status()
        print(f"UPDATED send_notification id={tool_id}")
    else:
        ...
        print(f"CREATED send_notification id={tool_id}")
